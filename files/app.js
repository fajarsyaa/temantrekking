...
/* ══════════════════════════════════════════════════════════════════
   HERO — foto latar, headline, penomoran trip, caption majalah
   ══════════════════════════════════════════════════════════════════ */
function initHero() {
  const { backgroundImage, heading, headingAccent, swipeLabel } = SITE.hero;

  document.getElementById('hero-bg').style.backgroundImage = `url('${backgroundImage}')`;
  document.getElementById('hero-heading').innerHTML =
    `${heading}<br><span style="color:#E2622A">${headingAccent}</span>`;
  document.getElementById('hero-swipe-label').textContent = swipeLabel;
...
// 3 caption ala majalah — dari 3 trip pertama
function renderHeroCaptions() {
  const el = document.getElementById('hero-captions');
  el.innerHTML = SITE.trips.slice(0, 3).map(trip => `
    <div>
      <p class="font-mono text-[10px] uppercase tracking-widest mb-2" style="color:#F2A24B">${trip.name} · ${trip.route}</p>
      <p class="text-sm leading-relaxed" style="color:rgba(244,241,232,0.75)">${trip.description.slice(0, 100).trim()}…</p>
    </div>
  `).join('');
}

// Penomoran vertikal di kanan hero — menandai trip mana yang sedang aktif
function renderHeroCounter() {
  const el = document.getElementById('hero-counter');
  el.innerHTML = SITE.trips.map((_, i) => `
    <span class="hero-count-digit font-display transition-all" data-index="${i}" style="color:rgba(244,241,232,0.35)">
      ${String(i + 1).padStart(2, '0')}
    </span>
  `).join('');
}

function updateHeroCounter(index) {
  document.querySelectorAll('.hero-count-digit').forEach(el => {
    const active = Number(el.dataset.index) === index;
    el.style.color = active ? '#E2622A' : 'rgba(244,241,232,0.35)';
    el.style.fontSize = active ? '1.5rem' : '1rem';
  });
}
...
function initTripCards() {
  ...
  SITE.trips.forEach((trip, i) => {
    ...
    card.className = `trip-card relative rounded-2xl overflow-hidden aspect-[4/5] select-none ${i === 0 ? 'active' : ''}`;
    card.dataset.index = i;
    card.innerHTML = `
      <img src="${trip.image}" alt="${escAttr(trip.name)}" loading="lazy"
           class="absolute inset-0 w-full h-full object-cover transition-transform duration-700 trip-card-img">

      <div class="absolute inset-0" style="background:linear-gradient(180deg, rgba(15,26,20,.12) 0%, rgba(15,26,20,.15) 35%, rgba(15,26,20,.92) 100%)"></div>

      <p class="absolute top-5 left-5 font-display text-lg" style="color:#F2A24B">${String(i + 1).padStart(2, '0')}</p>
      <span class="badge ${diffClass} absolute top-5 right-5">${trip.difficulty}</span>

      <div class="absolute bottom-0 inset-x-0 p-6">
        <p class="font-mono text-[10px] uppercase tracking-widest mb-2" style="color:#F2A24B">${trip.route}</p>
        <h3 class="font-display text-3xl uppercase leading-[0.9]" style="color:#F4F1E8">${trip.name}</h3>

        <div class="flex items-center gap-4 mt-3 font-mono text-[10px] uppercase" style="color:rgba(244,241,232,0.7)">
          <span>${trip.elevation}m</span>
          <span style="color:rgba(244,241,232,0.35)">·</span>
          <span>${trip.duration}H</span>
          <span style="color:rgba(244,241,232,0.35)">·</span>
          <span class="${trip.slotLeft <= 5 ? '' : ''}" style="color:${trip.slotLeft <= 5 ? '#F2A24B' : 'rgba(244,241,232,0.7)'}">${trip.slotLeft} slot</span>
        </div>

        <div class="flex items-end justify-between mt-4 pt-4" style="border-top:1px solid rgba(244,241,232,0.18)">
          <div>
            <p class="font-mono text-[9px] uppercase tracking-wider" style="color:rgba(244,241,232,0.55)">Berangkat</p>
            <p class="font-mono text-xs font-bold mt-0.5" style="color:#F4F1E8">${trip.nextDate}</p>
          </div>
          <p class="font-mono text-xs font-bold" style="color:#F2A24B">${priceLabel}</p>
        </div>
      </div>

      <div class="trip-card-bar absolute bottom-0 left-0"></div>
    `;
...
/* ══════════════════════════════════════════════════════════════════
   CTA — foto full-bleed mengikuti trip aktif + thumbnail trip lain
   ══════════════════════════════════════════════════════════════════ */
function renderCta(trip) {
  const bg = document.getElementById('cta-bg');
  if (bg) bg.style.backgroundImage = `url('${trip.image}')`;

  const thumbs = document.getElementById('cta-thumbs');
  if (!thumbs) return;
  thumbs.innerHTML = SITE.trips.map((t, i) => `
    <button class="cta-thumb w-14 h-14 lg:w-16 lg:h-16 rounded-full overflow-hidden shrink-0 transition-transform hover:-translate-y-1 hover:z-10"
            style="border:2.5px solid ${t === trip ? '#E2622A' : '#0F1A14'}"
            title="${escAttr(t.name)}" data-index="${i}">
      <img src="${t.image}" alt="${escAttr(t.name)}" class="w-full h-full object-cover">
    </button>
  `).join('');

  thumbs.querySelectorAll('.cta-thumb').forEach(btn => {
    btn.addEventListener('click', () => selectTrip(Number(btn.dataset.index)));
  });
}
